...
/**
 * Pins AppState.advanceOnchainBaselineForCompletedSpliceOut() (#316 review follow-up): a splice-out
 * that pays one of our own addresses (a self-send) raises the on-chain balance by its own settled
 * amount. The deposit detector's frozen baseline must absorb exactly that amount, not jump all the
 * way to the post-refresh balance — otherwise a genuinely separate deposit landing in the same
 * window would be silently swallowed into the baseline and never detected, which is the exact #316
...
class SpliceOutBaselineAdvanceDecisionTest {
    ...
    fun exactSelfSendSettlementCatchesBaselineUpToCurrent() {
        ...
        val result =
            AppState.advanceOnchainBaselineForCompletedSpliceOut(
                prevOnchainSats = 0L,
                currentSats = 50_000L,
                spliceAmountSats = 50_000L,
            )
