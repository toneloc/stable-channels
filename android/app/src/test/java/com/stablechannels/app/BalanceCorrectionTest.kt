...
class BalanceCorrectionTest {
    ...
    private fun sync(expected: Double, version: Long = 1) = TradeControlMessage.Sync(
        channelId, "lsp-has-a-different-id", expected, 999_999L, version, null
    )
    ...
    fun correctionClampsAnOverCapacityAllocationAndSurvivesReplayAndRestart() {
        ...
        db.close()
        db = DatabaseService(context)
        assertEquals(TradeControlApplyStatus.DUPLICATE,
    ...
    fun correctionRestoresATargetJustAboveTheWalletsCapacity() {
        ...
        assertEquals(TradeControlApplyStatus.APPLIED,
    ...
    fun correlatedTradeAcceptanceStillRequiresCapacityForItsStoredAllocation() {
        ...
        val trade = TradeProtocol.prepare(
            ...
            100_000L, "sell", 10.0, 0.000099, 0.1, 59.9, price,
        ...
        val message = TradeControlMessage.Sync(channelId, "7", trade.newExpectedUsd, 999_999L, 1,
            TradeCorrelation(trade.tradeId, paymentId, trade.requestHash))
        save(50.0, 55_000L, 58_000L)
        assertEquals(TradeControlApplyStatus.RETRY, db.applyCorrelatedTradeAcceptance(message).status)
        assertBooks(50.0, 55_000L, 0)
        assertTrue(db.tradeIsUnresolved(tradeRow))
        save(50.0, 55_000L, 100_000L)
        assertEquals(TradeControlApplyStatus.APPLIED, db.applyCorrelatedTradeAcceptance(message).status)
    ...
    fun correctionStillRetriesWhenNoPositiveBackingCanBeDerived() {
        ...
        assertEquals(TradeControlApplyStatus.RETRY,
    ...
    fun invalidOrUnrepresentablePricesNeverAdvanceTheVersion() {
        ...
        for (badPrice in listOf(0.0, -1.0, Double.NaN, Double.POSITIVE_INFINITY)) {
            assertEquals(TradeControlApplyStatus.INVALID,
    ...
    fun failedCorrectionRollsBackItsBooksAndVersionThenRetries() {
        save(50.0, 55_000L, 58_000L)
        db.writableDatabase.execSQL("CREATE TRIGGER refuse_sync BEFORE UPDATE ON channels BEGIN SELECT RAISE(ABORT, 'test'); END")
        assertThrows(SQLiteException::class.java) { db.applyUncorrelatedSyncIfNewer(sync(60.0), price) }
        assertBooks(50.0, 55_000L, 0)
    ...
    fun signedCorrectionWaitsForSpliceAccountingThenPublishesAndCachesTheLatestBooks() {
        ...
        val worker = Thread {
            started.countDown()
            try { assertTrue(processSync(state, sync(8.0))) } catch (t: Throwable) { failure.set(t) }
        }.apply { isDaemon = true }
        try {
            synchronized(lock) {
                worker.start()
                ...
                fun waitingForBooks(): Boolean = worker.state == Thread.State.BLOCKED &&
                ...
                assertTrue("SYNC must wait for the same lock as splice accounting", waitingForBooks())
        ...
        assertBooks(8.0, 8_000L, 1)
        assertEquals(8f, context.getSharedPreferences("balance_cache", Context.MODE_PRIVATE)
            .getFloat("cached_expected_usd", -1f), 0f)
    }
    ...
    fun failedSpliceAccountingLeavesTheOperationPendingAndRetriesExactlyOnce() {
        ...
        val state = stateWithBooks(10.0, 10_000L, 9_000L)
        val rowId = pendingSplice()
        db.writableDatabase.execSQL("CREATE TRIGGER refuse_splice BEFORE UPDATE ON channels BEGIN SELECT RAISE(ABORT, 'test'); END")
    ...
    private fun pendingSplice(): Long = db.recordPayment(
        "splice-payment", "splice_out", "sent", 1_000_000L, status = "pending", txid = txid
    )
    ...
    private fun stateWithBooks(expected: Double, backing: Long, receiver: Long): AppState =
        AppState(context).also { state ->
            ...
            (field(state, "_stableChannel") as MutableStateFlow<StableChannel>).value = StableChannel(
                channelId = channelId, userChannelId = "7", expectedUSD = USD(expected),
                backingSats = backing, stableReceiverBTC = Bitcoin(receiver), latestPrice = price
            )
    ...
    private fun processSync(state: AppState, message: TradeControlMessage): Boolean =
        AppState::class.java.getDeclaredMethod("processSignedSyncMessage", TradeControlMessage::class.java,
            String::class.java, Long::class.javaPrimitiveType).apply { isAccessible = true }
            .invoke(state, message, "sync-payment", TradeProtocol.RESULT_CONTROL_AMOUNT_MSAT) as Boolean
