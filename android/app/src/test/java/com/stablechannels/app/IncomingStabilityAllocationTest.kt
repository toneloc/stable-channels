...
class IncomingStabilityAllocationTest {
    ...
    fun rejectsInvalidBooksAndUnrepresentableTargets() {
        for (target in listOf(-1.0, Double.NaN, Double.POSITIVE_INFINITY, Double.MAX_VALUE)) {
            assertNull(StabilityService.backingAfterIncomingStability(9_000, target, 100_000.0, 500))
        }
        ...
        assertNull(StabilityService.backingAfterIncomingStability(0, Long.MAX_VALUE.toDouble(), 100_000_000.0, 1))
    }

    @Test
    fun capsBeforeAddingToAvoidOverflow() {
        assertEquals(10_000L, StabilityService.backingAfterIncomingStability(
            9_000, 10.0, 100_000.0, Long.MAX_VALUE))
    }
    ...
    fun usesWholeSatsEvenForSubCentTargets() {
        ...
        assertEquals(20L, StabilityService.backingAfterIncomingStability(20, 0.0099, 100_000.0, 500))
    }
}
