...
class PaymentFailureRecorderTest {
    ...
    private fun prepare(tradeId: String = "ef".repeat(32)): PreparedTrade =
    ...
    private fun recordFailure(pid: String = paymentId) =
        PaymentFailureRecorder.record(db, pid, "RETRIES_EXHAUSTED") { null }

    private fun appStateWithDatabase() =
    ...
    fun foregroundEventPublishesFailedTradeEvenBeforePendingMapRegistration() {
        ...
        val handler =
            AppState::class.java.getDeclaredMethod("handleEvent", Event::class.java).apply {
                isAccessible = true
            }
        handler.invoke(
            state,
            Event.PaymentFailed(paymentId, null, PaymentFailureReason.RETRIES_EXHAUSTED),
        )
    ...
    fun foregroundOrdinaryFailuresAreReadableAndKeptSeparateByPaymentId() {
        ...
        handler.invoke(
            state,
            Event.PaymentFailed(otherPayment, null, PaymentFailureReason.PAYMENT_EXPIRED),
        )
    ...
    fun backgroundSuccessRepairsPendingHistoryAndSurvivesRelaunch() {
        ...
        val repaired =
            LightningPaymentRecovery.reconcilePending(db) { id ->
    ...
    fun failedUnattachedFeeIsRecoveredWithItsReason() {
        ...
        val result =
            PaymentFailureRecorder.record(db, paymentId, "ROUTE_NOT_FOUND") { trade.feeMsat }
        assertTrue(result.isTrade)
        assertTrue(result.tradeOutcome!!.sendFailed)
        ...
        for ((index, code) in codes.withIndex()) {
            ...
            val bytes =
                JSONObject()
                    ...
                    .toByteArray()
            val rejection =
                TradeProtocol.parseSignedControl(bytes, "provider") { _, sig, peer ->
            ...
            assertEquals(
                ...
                db.applyTradeRejection(rejection).status,
            )
    ...
    fun persistenceFailureEscapesSoTheEventWillNotBeAcknowledged() {
        ...
        db.writableDatabase.execSQL(
            "CREATE TRIGGER refuse_failure BEFORE UPDATE ON trades BEGIN SELECT RAISE(ABORT, 'test'); END"
        )
    ...
    private fun rejection(trade: PreparedTrade, code: String) =
