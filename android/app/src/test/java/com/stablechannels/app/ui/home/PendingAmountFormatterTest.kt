...
/**
 * Pins PendingAmountFormatter (#316 follow-up review finding): PendingRow's amount and the
 * "Move ... to Lightning" label switched from BTC to USD display, but showed nothing at all when
 * the price feed was momentarily unavailable (btcPrice <= 0) instead of a graceful fallback. Both
 * now fall back to a BTC figure in that case rather than silently dropping the amount.
 *
...
class PendingAmountFormatterTest {
    ...
    fun moveToLightningLabelUsesUsdWhenPriceIsAvailable() {
        val label = PendingAmountFormatter.moveToLightningLabel(spendableSats = 100_000L, btcPrice = 50_000.0)
    ...
    fun moveToLightningLabelFallsBackToBtcWhenPriceIsUnavailable() {
        val label = PendingAmountFormatter.moveToLightningLabel(spendableSats = 100_000L, btcPrice = 0.0)
