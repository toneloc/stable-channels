...
class StabilizationPolicyTest {
    @Test
    fun reserveExplanationIsOnlyInOverLimitFeedback() {
    ...
    fun canonicalVectors() {
        ...
        for ((snapshot, maximum) in vectors) {
            ...
            fun prepare(cents: Long): PreparedTrade? {
                ...
                val sc =
                    StableChannel(
                        ...
                        userChannelId = "7",
                        expectedUSD = USD(snapshot.expectedUsd),
                        stableReceiverBTC = Bitcoin(snapshot.receiverSats),
                        backingSats = snapshot.backingSats,
                    )
                return TradeProtocol.prepare(
                    sc,
                    snapshot.spendableSats,
                    "sell",
                    amount,
                    (amount - fee) / snapshot.price,
                    fee,
                    snapshot.expectedUsd + (amount - fee),
                    snapshot.price,
                )
            }
            ...
            val above =
                try {
                    prepare(maximum + 1)
                } catch (_: TradeValidationException) {
                    null
                }
    ...
    @Test
    fun integerBoundaryAndSmallBalances() {
    ...
    @Test
    fun changedSnapshotRejectsOldMaximum() {
    ...
    @Test
    fun directPrepareCannotBypassLimitAndReductionsStayAllowed() {
        val sc =
            StableChannel(
                channelId = "ab".repeat(32),
                userChannelId = "7",
                expectedUSD = USD(99.0),
        ...
        assertNotNull(
            TradeProtocol.prepare(sc, 100_000, "buy", 1.0, 0.0000099, 0.01, 98.0, 100_000.0)
        )
        assertNotNull(
            TradeProtocol.prepare(sc, 100_000, "buy", 99.0, 0.0009801, 0.99, 0.0, 100_000.0)
        )
    }
}
