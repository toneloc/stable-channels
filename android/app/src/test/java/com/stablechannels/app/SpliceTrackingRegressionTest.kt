...
class SpliceTrackingRegressionTest {
    ...
    @Test fun unrelatedTransactionDoesNotMatchFailedOrUnassignedSpliceIn() {
    ...
    @Test fun failedExactTransactionDoesNotPassPendingCheck() {
        db.recordPayment(null, "splice_out", "sent", 1_000_000, status = "failed", txid = txid)
        assertFalse(db.hasPendingSpliceFor(txid))
        assertFalse(db.completeSplice(txid))
    }

    @Test fun unrelatedConfirmationCannotClaimANewerPendingOperationOrShowSuccess() {
    ...
    @Test fun failedOperationCannotShowSuccess() {
        val rowId = db.recordPayment(null, "splice_out", "sent", 1_000_000, status = "failed", txid = txid)
        val state = state()
    ...
    @Test fun failedHistoryWriteKeepsCompletionRetryable() {
        val rowId = db.recordPayment(null, "splice_out", "sent", 1_000_000, status = "pending", txid = txid)
        val state = state()
        db.writableDatabase.execSQL("CREATE TRIGGER reject_completion BEFORE UPDATE OF status ON payments BEGIN SELECT RAISE(ABORT, 'test'); END")
        assertEquals("DEFERRED", complete(state, rowId))
    ...
    @Test fun capturedRowCannotCompleteAnotherRowsTransaction() {
        val first = db.recordPayment(null, "splice_in", "received", 1_000_000, status = "pending", txid = "other")
    ...
    private fun state(): AppState = AppState(context).also { state ->
        ...
        (field(state, "_stableChannel") as MutableStateFlow<StableChannel>).value = StableChannel(
            channelId = channelId, userChannelId = "7", expectedUSD = USD(10.0),
