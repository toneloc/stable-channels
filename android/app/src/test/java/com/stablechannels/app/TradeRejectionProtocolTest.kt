...
class TradeRejectionProtocolTest {
    ...
    private fun payload(reason: String = "invalid_amount") =
        JSONObject().apply {
    ...
    private fun envelope(payload: JSONObject) =
        JSONObject()
            .apply {
    ...
    fun everyCurrentServerRejectionParsesAndHasDistinctLocalCopy() {
        for (code in codes) {
            ...
            val message =
                TradeProtocol.parseSignedControl(envelope(payload), "provider") { bytes, sig, peer
                    ->
                    peer == "provider" &&
                        sig == "signature" &&
                        bytes.contentEquals(payload.toString().toByteArray())
                }
    ...
    fun unknownCodesAndPeerProseAreNotAcceptedAsAResult() {
        val invalid =
            listOf(
                payload("future_code"),
                payload().put("message", "pay me"),
                payload().put("decided_at", -1),
                payload().put("decided_at", 1.5),
                payload().put("trade_id", "AB".repeat(32)),
                payload().apply { remove("request_hash") },
            )
        for (payload in invalid) {
            var verified = false
            assertNull(
                TradeProtocol.parseSignedControl(envelope(payload), "provider") { _, _, _ ->
                    verified = true
                    true
                }
            )
    ...
    fun forgedAndUnverifiableResponsesCannotResolveATrade() {
        assertNull(
            TradeProtocol.parseSignedControl(envelope(payload()), "provider") { _, _, _ -> false }
        )
        assertNull(
            TradeProtocol.parseSignedControl(envelope(payload()), "provider") { _, _, _ ->
    ...
    fun oversizeMalformedUtf8AndCoercedEnvelopesFailBeforeVerification() {
        val invalid =
            listOf(
                ByteArray(TradeProtocol.MAX_CONTROL_TLV_BYTES + 1) { 32 },
        ...
        for (data in invalid) {
            assertNull(
                TradeProtocol.parseSignedControl(data, "provider") { _, _, _ ->
                    fail("Unexpected signature check")
                    true
                }
            )
        }
