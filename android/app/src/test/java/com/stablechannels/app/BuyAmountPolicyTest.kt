...
class BuyAmountPolicyTest {
    @Test
    fun fractionalCentBalanceDoesNotRoundMaxUp() {
    ...
    @Test
    fun exactCentBalancesKeepTheirLastCentAndHaveNoPercentageCap() {
    ...
    @Test
    fun displayedMaximumIsAcceptedAcrossCentBoundaries() {
        for (cents in 1..10_000) {
            val exact = cents / 100.0
            for (balance in
                listOf(exact, Math.nextDown(exact), Math.nextUp(exact), exact + 0.006)) {
    ...
    @Test
    fun invalidBalancesFailClosed() {
        for (balance in
            listOf(-1.0, 0.0, Double.NaN, Double.POSITIVE_INFINITY, Double.NEGATIVE_INFINITY)) {
    ...
    @Test
    fun balancesBelowOneCentHaveNoWholeCentMaximum() {
    ...
    @Test
    fun invalidOrOverLimitAmountsAreRejected() {
        for (amount in
            listOf(
                -1.0,
                0.0,
    ...
    @Test
    fun lowerLiveBalanceRejectsPreviouslySelectedMaximum() {
