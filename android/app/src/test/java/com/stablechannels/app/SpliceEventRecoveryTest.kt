...
class SpliceEventRecoveryTest {
    ...
    /** A move out is a channel spend: the guard needs a running node listing the channel at par and a fresh price. */
    @Suppress("UNCHECKED_CAST")
    private fun installNode(state: AppState, receiver: Long = 10_000): TestNode {
        val node = TestNode().also { it.channels = listOf(channel(cid = channel, receiver = receiver)) }
    ...
    @Test fun aMoveOutCompletesEndToEndFromBeginThroughNegotiationAndConfirmation() {
        ...
        try {
            routeEsploraTo(state, server) { MockResponse().setBody("{\"confirmed\":false}") }
            state.beginSpliceOut(1_000, "addr", 100_000.0)
            node.channels = listOf(channel(cid = channel, receiver = 9_000)) // the 1,000-sat move left
