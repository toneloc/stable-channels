...
object WalletErrorMessages {
    fun paymentFailure(reason: PaymentFailureReason?): String =
        when (reason) {
    ...
    fun paymentFailureCode(code: String?): String =
        paymentFailure(PaymentFailureReason.entries.firstOrNull { it.name == code })

    fun operation(error: Exception, fallback: String): String =
        when (error) {
            ...
            is NodeException.InvalidInvoice ->
                "This invoice is invalid or expired. Ask the recipient for a new one."
            is NodeException.InvalidOffer ->
                "This offer is invalid. Ask the recipient for a new one."
            ...
            is NodeException.InvalidFeeRate ->
                "The fee rate is invalid. Refresh the fee estimate and try again."
            ...
            is NodeService.NodeServiceError ->
