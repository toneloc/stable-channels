...
/**
 * Shared Android pricing policy. Direct BTC/USD books define the index. BTC/USDT books are queried
 * only after direct USD quorum fails and are normalized through a validated USDT/USD peg.
 */
object PriceOracle {
    ...
    val BITCOIN_USDT_FEEDS =
        listOf(
            ...
            PriceFeedConfig(
                ...
                listOf("result", "list", "0", "lastPrice"),
            ),
            PriceFeedConfig(
                "Huobi BTC/USDT",
    ...
    fun validateUsdtPeg(prices: List<NamedPrice>): Pair<Double, List<NamedPrice>> {
        ...
        val initialExchangeMedian =
            median(exchangePrices.map { it.value })
        ...
        val acceptedExchangeMedian =
            median(agreeingExchangePrices.map { it.value })
        ...
        if (abs(acceptedExchangeMedian - 1.0) > MAXIMUM_USDT_PEG_DEVIATION_FROM_DOLLAR) {
            throw PriceOracleException(
                "Exchange USDT/USD consensus is outside the allowed peg band"
            )
        }
    ...
    private fun relativeDeviation(value: Double, reference: Double): Double {
        if (!value.isFinite() || !reference.isFinite() || reference <= 0)
