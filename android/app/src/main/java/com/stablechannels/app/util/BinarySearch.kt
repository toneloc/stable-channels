...
/**
 * Finds the index of the first element that satisfies the given predicate, assuming elements where
 * predicate returns false precede elements where predicate returns true.
 */
...
/**
 * Finds the nearest element to the given target key in a list sorted by the key, using a
 * caller-provided distance metric.
 */
...
/**
 * Convenience overload for Long keys (e.g. timestamps, sats). Distance is computed directly in
 * 64-bit integer arithmetic without floating-point conversions. Note: Assumes difference fits
 * within Long without overflow (e.g. timestamps or satoshi amounts). For arbitrary domains with
 * extreme opposing bounds (e.g. Long.MIN_VALUE and Long.MAX_VALUE), callers should supply a custom
 * distance closure.
 */
...
/**
 * Convenience overload for Int keys. Distance is computed directly in integer arithmetic without
 * floating-point conversions. Note: Assumes difference fits within Int without overflow.
 */
