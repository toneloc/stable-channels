...
/**
 * Polls Esplora /tx/{fundingTxid}/outspend/{vout} to resolve the close transaction ID for a
 * force-closed channel. Runs in background until the close TX is found or budget expires.
 */
class CloseTxidResolver(
    ...
    suspend fun resolve(
        ...
        withContext(Dispatchers.IO) {
            Log.d(
                TAG,
                "Starting close TX resolution for paymentId=$paymentId, fundingTxid=$fundingTxid:$vout",
            )
