...
object StabilizationPolicy {
    fun backingCap(postFeeSpendable: Long): Long? {
        ...
        val percent =
            (postFeeSpendable / 100) * Constants.MAX_STABLE_ALLOCATION_PERCENT +
                ((postFeeSpendable % 100) * Constants.MAX_STABLE_ALLOCATION_PERCENT) / 100
        return percent
    }

    fun clientLimit(postFeeSpendable: Long): Long? =
        backingCap(postFeeSpendable)
            ?.let { it - Constants.CLIENT_SAFETY_MARGIN_SATS }
            ?.takeIf { it > 0L }

    fun maximumMessage(cents: Long) =
        String.format(Locale.US, "Maximum additional trade: $%.2f", cents / 100.0)
...
data class StabilizationSnapshot(
    ...
    private fun fits(orderCents: Long, searching: Boolean): Boolean {
        ...
        if (
            !required.isFinite() ||
                required > (receiverSats - backingSats).coerceAtLeast(0).toDouble()
        )
            return false
        val target =
            TradeProtocol.normalizeExpectedUsd(
                expectedUsd + (amount - amount * Constants.STABLE_CHANNEL_TRADE_FEE_RATE)
            )
        if (target < expectedUsd || (!searching && target == expectedUsd)) return false
        val fee =
            (TradeProtocol.expectedTradeFeeMsat(expectedUsd, target, price) ?: return false) / 1000
        ...
        if (
            searching &&
                backingSats == 0L &&
                floor(target / price * Constants.SATS_IN_BTC) ==
                    floor(expectedUsd / price * Constants.SATS_IN_BTC)
        )
            return true
        val backing =
    ...
    fun maxOrderCents(): Long {
        ...
        val cents =
            floor(
                (receiverSats - backingSats).coerceAtLeast(0).toDouble() / Constants.SATS_IN_BTC *
                    price *
                    100
            )
