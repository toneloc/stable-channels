...
object SpliceEventRecorder {
    ...
    internal fun track(db: SQLiteDatabase, rowId: Long, userChannelId: String, channelId: String, previousFundingTxid: String?) {
        require(userChannelId.isNotBlank() && channelId.isNotBlank())
        db.insertOrThrow("splice_operations", null, ContentValues().apply {
    ...
    fun record(
        ...
        db.transaction {
            ...
            val seen = rawQuery("SELECT 1 FROM splice_events WHERE event_key = ?", arrayOf(key))
                .use { it.moveToFirst() }
            if (!seen) {
                val rowId = findOperation(db, kind, userChannelId, channelId, funding?.txid)
                insertOrThrow("splice_events", null, ContentValues().apply {
                    put("event_key", key)
                    put("event_type", kind)
                    put("user_channel_id", userChannelId)
                    put("channel_id", channelId)
            ...
            if (funding != null) {
                update("splice_events", ContentValues().apply {
                    put("txid", funding.txid)
                    put("vout", funding.vout.toLong())
                }, "event_key = ? AND txid IS NULL", arrayOf(key))
            }
    ...
    private fun findOperation(
        db: SQLiteDatabase, kind: String, userChannelId: String, channelId: String, txid: String?
    ): Long? {
        ...
        val exact = db.rawQuery("""
            ...
            LIMIT 2
        """, arrayOf(txid.orEmpty(), userChannelId)).use { c ->
        ...
        val trackedArgs = buildList {
            add(userChannelId); add(channelId)
            if (kind == "ready") add(txid.orEmpty())
            add(txid.orEmpty())
        }.toTypedArray()
        val tracked = db.rawQuery("""
        ...
        val legacy = db.rawQuery("""
            ...
            LIMIT 2
        """, arrayOf(
            (System.currentTimeMillis() / 1000 - DatabaseService.PENDING_SPLICE_WITHOUT_TXID_TIMEOUT_SECS).toString(),
