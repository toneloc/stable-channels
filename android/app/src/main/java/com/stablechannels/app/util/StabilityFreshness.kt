 ...
 *
 * A stability payment may only be sent when LDK completed a Lightning-wallet chain sync within
 * [Constants.STABILITY_MAX_LIGHTNING_SYNC_AGE_SECS]. Paying on a stale chain tip understates
 * outbound HTLC expiry, which LDK later force-closes on.
 *
 * Keep in sync with `lightning_sync_is_fresh` in `src/stable.rs` and `StabilityFreshness` in the
 * iOS app.
 */
object StabilityFreshness {

    /**
     * True when [latestSyncTimestampSecs] exists, is not in the future, and is at most [maxAgeSecs]
     * old (exactly [maxAgeSecs] old is accepted). A missing timestamp means the wallet has never
     * synced; a future one means clock skew — both block the send.
     */
    fun isFresh(
