            ...
            Card(
                ...
                Column(modifier = Modifier.padding(16.dp)) {
                    ...
                    val usdVal =
                        payment.amountUSD
                            ?: run {
                                ...
                                price?.let {
                                    (payment.amountSats.toDouble() / Constants.SATS_IN_BTC) * it
                                }
                            }
                    val amountStr =
                    ...
                    payment.explorerTxid()?.let { txid ->
                        ...
                        val displayTxid =
                            if (txid.length > 16) txid.take(8) + "..." + txid.takeLast(8) else txid
                    ...
                    payment.address?.let { addr ->
                        ...
                        val displayAddr =
                            if (addr.length > 16) addr.take(8) + "..." + addr.takeLast(8) else addr
                    ...
                    if (payment.shouldShowConfirmationProgress() || payment.confirmations > 0) {
                        ...
                        val confirmationsLabel =
                            if (payment.confirmations >= required) {
...
private fun PaymentRecord.explorerTxid(): String? {
    ...
    if (paymentType == "onchain") {
        val fromPaymentId = paymentId?.removePrefix("onchain_receive_")?.trim()
