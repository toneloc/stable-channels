...
/**
 * STABILITY_PAYMENT_V1 payload/envelope codec shared by both send paths and both receive paths.
 * Mirrors stable.rs build/parse_stability_payment_payload exactly so the wire format stays
 * interchangeable with the Rust peers.
 */
object StabilityPaymentProtocol {
    const val DIRECTION_USER_TO_LSP = "user_to_lsp"
    const val DIRECTION_LSP_TO_USER = "lsp_to_user"

    fun newSettlementId(): String =
    ...
    fun buildPayload(
        ...
        if (
            !TradeProtocol.isCanonicalIdentifier(settlementId) ||
    ...
    fun buildSignedEnvelope(
        ...
        val payload =
            buildPayload(
                ...
                channelId,
                amountMsat,
                DIRECTION_USER_TO_LSP,
                expectedUsd,
                now,
                now + Constants.STABILITY_PAYMENT_TTL_SECS,
            ) ?: return null
    ...
    /**
     * Validate an inbound SIGNED_STABILITY_TLV record against the received keysend. Check order
     * mirrors user.rs handle_signed_stability_payment_received.
     */
    fun validateInbound(
        data: ByteArray,
        ...
        val payloadStr =
            envelope.opt("payload") as? String
                ?: return SignedSettlementValidation.Invalid("envelope")
        val signature =
            envelope.opt("signature") as? String
                ?: return SignedSettlementValidation.Invalid("envelope")
        val payment =
            parsePayload(payloadStr) ?: return SignedSettlementValidation.Invalid("fields")
