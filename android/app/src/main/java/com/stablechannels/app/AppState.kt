...
class AppState(private val context: Context) : ViewModel() {
     ...
     *  Signed SYNC/trade results, splice reconciliation, and channel saves/reloads share this
     *  lock too, so a committed correction cannot be overwritten by an older snapshot. */
    ...
    private fun processSignedSyncMessage(
        ...
        paymentHash: String,
        amountMsat: Long
    ): Boolean = synchronized(booksLock) {
            ...
            val accounted = try {
                synchronized(booksLock) {
                    ...
                    if (!price.isFinite() || price <= 0.0) {
                        ...
                        val result = db.reconcileOutgoingBacking(
                        ...
                        publishBooksFromDB(
                            lastStabilityPayment = if (result != null) System.currentTimeMillis() / 1000 else null,
                            recomputeNative = true
                        )
                        saveChannelToDB(preserveBacking = true)
                        true
                ...
                AuditService.log("SPLICE_RECONCILE_DEFERRED", mapOf(
                    "txid" to txid, "reason" to "persistence_failed", "error" to (e.message ?: "")
                ))
                false
            }
    ...
    fun updateStableBalances() {
        val price = priceService.currentPrice.value
        _stableChannel.update {
            StabilityService.updateBalances(it, nodeService.channels, _onchainBalanceSats.value, price)
        }
    }
