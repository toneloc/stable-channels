...
#             Introduction: This is UTXOracle.py CLN Plugin           

###############################################################################  

# This is a modfication of code found here - https://utxo.live/oracle/ - to make 
# it work with Core Lightning

# This python CLN plugin estimates the daily USD price of bitcoin using only
...
print("UTXOracle CLN plugin version 6\n")

### Modification: additional functions

def var_int(data, offset):
    """Extract variable length integer and its size from data starting at offset."""
...
def parse_transaction(data, offset, outputs):
    """Parse a transaction and return the offset after the transaction."""
    # Start of transaction: version (4 bytes)
    offset += 4

    # Input count
    input_count, size = var_int(data, offset)
    offset += size

    # Parse inputs
    for _ in range(input_count):
        # TXID (32 bytes) and output index (4 bytes)
        offset += 36

        # Signature script
        script_length, size = var_int(data, offset)
        offset += size + script_length

        # Sequence
        offset += 4

    # Output count
    output_count, size = var_int(data, offset)
    offset += size
    ...
    for _ in range(output_count):
        ...
        outputs.append(value)  # Adding the value to the outputs list
        offset += 8

        # PK Script
        pk_script_length, size = var_int(data, offset)
        offset += size + pk_script_length
...
def get_raw_block_utxo_values(hex_block):
    data = bytes.fromhex(hex_block)
    
    # Skip over block header (80 bytes)
    offset = 80

    # Get number of transactions
    tx_count, size = var_int(data, offset)
    offset += size
    ...
    for _ in range(tx_count):
        offset = parse_transaction(data, offset, outputs)
...
# configuration options for lighting-cli
datadir      = ""
rpcuser      = "" 
rpcpassword  = ""
rpcookiefile = ""
...
# add the configuration options to the lightning-cli call
lightning_cli_options = []
if datadir      != "":
    lightning_cli_options.append('-datadir='+ datadir)
if rpcuser      != "":
    lightning_cli_options.append("-rpcuser="+ rpcuser)
if rpcpassword  != "":
    lightning_cli_options.append("-rpcpassword="+ rpcpassword)
if rpcookiefile != "":
    lightning_cli_options.append("-rpcookiefile="+ rpcookiefile)
if rpcconnect   != "":
    lightning_cli_options.append("-rpcconnect="+ rpcconnect)
if rpcport      != "":
    lightning_cli_options.append("-rpcport="+ rpcport)
if conf         != "":
    lightning_cli_options.append("-conf="+ conf)
...
# Stable Channel modfication = modified this function to work with "lightning-cli"
import subprocess #a built in python library for sending command line commands
def Ask_Node(command):

    for o in lightning_cli_options:
        command.insert(0,o)
    command.insert(0,"lightning-cli --testnet")
    ...
    try:  #python try is used when we need to deal with errors after
        ...
        print("Error connecting to your Lightning node. Trouble shooting steps:\n")
...
# Stable Channels modificaiton = change 'getblockcount' to 'getchaininfo'
getchaininfo_result = Ask_Node(['getchaininfo'])
block_count_b = int(getchaininfo_result["blockcount"])            

# Stable Channels modificaiton = change 'getblockheader' to TODO
# Get raw block; extract header
block_header_b = Ask_Node(['getblockheader', block_count_b[:64],'true'])
...
# Stable Channels modification = change 'getblockhash' to 'getrawblockbyheight'
getrawblockbyheight_result = Ask_Node(['getrawblockbyheight',str(price_day_block_estimate)])
block_hash_b = getrawblockbyheight_result["blockhash"]

# Stable Channels modification = change 'getblockheader' to TODO
...
while block_jump_estimate >6 and block_jump_estimate != last_last_estimate:
    ...
    # Stable Channels modification = change 'getblockhash' to TODO
...
while target_day_of_month == day_of_month:
    ...
    hex_block = Ask_Node(['getrawblockbyheight',str(block_height)]) 

    outputs = get_raw_block_utxo_values(hex_block)
